...
def do_run(args, device=None, is_colab=False, model_config=None):
  ...
  if args.ViTB32 is True: clipLoad('ViT-B/32')
  if args.ViTB16 is True: clipLoad('ViT-B/16')
  if args.ViTL14 is True: clipLoad('ViT-L/14')
  if args.ViTL14_336 is True: clipLoad('ViT-L/14@336px')
  if args.RN50 is True: clipLoad('RN50')
  if args.RN50x4 is True: clipLoad('RN50x4')
  if args.RN50x16 is True: clipLoad('RN50x16')
  if args.RN50x64 is True: clipLoad('RN50x64')
  if args.RN101 is True: clipLoad('RN101')
  ...
  if args.key_frames:
    ...
    angle_series, zoom_series, translation_x_series, translation_y_series, translation_z_series,
    rotation_3d_x_series, rotation_3d_y_series, rotation_3d_z_series) = processKeyFrameProperties(
        args.max_frames, args.interp_spline, args.angle, args.zoom, 
        args.translation_x, args.translation_y, args.translation_z, 
        args.rotation_3d_x, args.rotation_3d_y, args.rotation_3d_z)
  ...
  if (args.animation_mode == "3D") and (args.midas_weight > 0.0):
      (midas_model, midas_transform, midas_net_w, midas_net_h,
       midas_resize_mode, midas_normalization) = init_midas_depth_model(args.midas_depth_model, model_path=args.model_path, device=device)
  for frame_num in range(args.start_frame, args.max_frames):
      ...
      if args.animation_mode == "3D":
        if frame_num > 0:
          ...
          next_step_pil = do_3d_step(img_filepath, frame_num, midas_model, midas_transform, translations={
      ...
      if args.perlin_init:
          if args.perlin_mode == 'color':
              init = create_perlin_noise([1.5**-i*0.5 for i in range(12)], 1, 1, False, side_x=args.side_x, side_y=args.side_y)
              init2 = create_perlin_noise([1.5**-i*0.5 for i in range(8)], 4, 4, False, side_x=args.side_x, side_y=args.side_y)
          elif args.perlin_mode == 'gray':
            init = create_perlin_noise([1.5**-i*0.5 for i in range(12)], 1, 1, True, side_x=args.side_x, side_y=args.side_y)
            init2 = create_perlin_noise([1.5**-i*0.5 for i in range(8)], 4, 4, True, side_x=args.side_x, side_y=args.side_y)
          else:
            init = create_perlin_noise([1.5**-i*0.5 for i in range(12)], 1, 1, False, side_x=args.side_x, side_y=args.side_y)
            init2 = create_perlin_noise([1.5**-i*0.5 for i in range(8)], 4, 4, True, side_x=args.side_x, side_y=args.side_y)
          ...
          with torch.enable_grad():
              ...
              for model_stat in model_stats:
                for i in range(args.cutn_batches):
                    ...
                    dists = dists.view([eval(args.cut_overview)[1000-t_int]+eval(args.cut_innercut)[1000-t_int], n, -1])
      ...
      for i in range(args.n_batches):
          ...
          if args.perlin_init:
              init = regen_perlin(perlin_mode=args.perlin_mode, device=device, batch_size=args.batch_size)
                  ...
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      if j % args.display_rate == 0 or cur_t == -1:
                        ...
                        if(args.console_preview): 
                          output = climage.convert(f'{args.batchFolder}/progress.png', width=args.console_preview_width)
                          print(output)
                        # display.clear_output(wait=True)
                      ...
                      if cur_t == -1:
                        if frame_num == 0:
                          save_settings(args, batchFolder=args.batchFolder, batch_name=args.batch_name, batchNum=args.batchNum)
                        if args.animation_mode != "None":
                        ...
                        if args.animation_mode == "3D":
                          ...
                          if args.vr_mode:
                            generate_eye_views(args.TRANSLATION_SCALE, args.batchFolder, filename, frame_num, midas_model, midas_transform, 
                                               vr_eye_angle=args.vr_eye_angle, vr_ipd=args.vr_ipd, device=device, args=args)
