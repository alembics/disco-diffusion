...
if pargs.animation_mode == "Video Input":
    ...
    subprocess.run(f'ffmpeg -i {pargs.video_init_path} -vf f{vf} -vsync -vfr -q:v 2 -loglevel error -stats {videoFramesFolder}/%04d.jpg'.split(" "), stdout=subprocess.PIPE).stdout.decode("utf-8")
...
if pargs.resume_run:
    ...
    while (
        os.path.isfile(f"{batchFolder}/{pargs.batch_name}({batchNum})_settings.txt") is True or os.path.isfile(f"{batchFolder}/{pargs.batch_name}-{batchNum}_settings.txt") is True
    ):
        batchNum += 1
